...
import { readableStreamToString, LuccaCleemyExpensesError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LuccaCleemyExpensesError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LuccaCleemyExpensesError(e, e.response?.data)
                }
                if (innerError instanceof LuccaCleemyExpensesError) {
                    // Got "LuccaCleemyExpensesError" from the above try block
                    throw innerError;
                }
